...
public class SpecsController {
    ...
    public String viewHomePage(@PathVariable("id") String id, Model model) {
       ...
       // model.addAttribute("specs", specsRepository.findAll());
		model.addAttribute("specs",mongoTemplate.find(getSpecs,Specs.class));
        if(!id.isEmpty()) {
        	System.out.println("ID IS"+id);
        	Orders order1= orderRepository.findById(new ObjectId(id)).orElseThrow(() -> new IllegalArgumentException("Invalid user Id:" + id));
        	List<Specs> ListOfSpecs  = new ArrayList<>();
	...
	//	clearAllCaches();
	    ...
	    Specs spec = specsRepository.findById(_id)
	    ...
	    Product prodAv = mongoTemplate.findOne(getProduct,Product.class);
	    ...
	    Orders order = orderRepository.save(ord);
	...
	public String addToCart2(@PathVariable("id") String id,@PathVariable("id2") String id2, Model model,Orders order,RedirectAttributes redirAttrs) {
		System.out.println("Entered");
		ObjectId _id =new ObjectId(id);
	    Specs spec = specsRepository.findById(_id)
	      .orElseThrow(() -> new IllegalArgumentException("Invalid user Id:" + _id));
	    ...
	    Orders ords = orderRepository.findById(new ObjectId(id2))
